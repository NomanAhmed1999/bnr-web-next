...
export default function MainComponent() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
          try {
            const response = await fetch(`https://www.test.dc3worx.ca/api/website-content/`);
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            setCurrentWordIndex((currentWordIndex + 1) % data.hero.length);
        }, 2000);
                ...
                <div className="container mx-auto px-4 flex flex-col md:flex-row items-center">
                    <div className="md:w-1/2 text-center md:text-left mb-10 md:mb-0">
                        <h2 className="text-4xl md:text-5xl font-bold mb-4">
                            <span className="inline-block transition-all duration-500 ease-in-out" style={{ color: data.hero[currentWordIndex].color }}>
                                {data.hero[currentWordIndex].word}
                            </span>{' '}
            ...
            <div className="bg-gray-100 py-16">
                <div className="container mx-auto px-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        {data.supportingProducts.map((product, index) => (
